      ...
      <el-col :span="7" class="login-card">
        ...
        <el-form :model="loginForm" :rules="rules" ref="loginForm" label-width="21%" class="loginForm">
          ...
          <el-form-item class="btn-ground">
            ...
            <el-button type="primary" @click="handinForm()">注册</el-button>
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
...
function submitForm() {
  loginForm.value.validate((valid) => {
    if (valid) {
      // 表单验证成功
      this.$axios.post('http://10.249.69.159:8080/login', loginForm.value).then(response => {
        // 拿到结果
...
function handinForm() {
  loginForm.value.validate((valid) => {
    if (valid) {
      // 表单验证成功
      this.$axios.post('http://10.249.69.159:8080/register', loginForm).then(response => {
        // 拿到结果
        let res = response.data;
        let message = res.message;
        // 判断结果
        if (res.code === 1) {
          // 登陆成功
          ElMessage.success(message);
          // 保存token，跳转页面,res.data就是服务器返回的token
          let token = res.data;
          window.sessionStorage.setItem('token', token);
          router.push('/record');
        } else {
